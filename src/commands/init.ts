...
export async function runInit(args: string[]): Promise<void> {
  ...
  // For named targets, warn if directory exists and is non-empty
  if (targetName && existsSync(targetDir)) {
    const entries = await readdir(targetDir);
    if (entries.length > 0) {
      const proceed = await p.confirm({
        message: `Directory ${targetName}/ already exists and is not empty. Continue?`,
        initialValue: false,
      });
  ...
  if (setupType === "git") {
    await initFromGit(targetDir, targetName);
  } else {
    await initFromBuiltin(targetDir, targetName);
  }
}

// ─── Git Template Path ──────────────────────────────────────────────────────

async function initFromGit(targetDir: string, targetName: string | undefined): Promise<void> {
